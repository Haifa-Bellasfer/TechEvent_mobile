...
public class CartService {
    ...
    public void ajoutTask(Ticket ta) {
        ...
        String Url = Session.url_server + "cart/add?user_id=+" + ta.getUser_id() + "&event=" + ta.getEvent().getId_event() + "&status=" + ta.getStatus();
        con.setUrl(Url);
    ...
    public ArrayList<Ticket> parseListTaskJson(String json) {
        ...
        try {
            ...
            for (Map<String, Object> obj : list) {
                ...
                if (idu == Session.current_user.getId_user()) {
                    e.setEvent(ev);
                    e.setId_ticket(id);
                    listTasks.add(e);
                }
    ...
    public ArrayList<Ticket> getList2() {
        ...
        ConnectionRequest con = new ConnectionRequest();
        con.setUrl(Session.url_server + "cart/AllTicket");
    ...
    public void delete(int id) {
        ...
        ConnectionRequest con = new ConnectionRequest();
        String Url = Session.url_server + "cart/del/" + id;
        con.setUrl(Url);
...
//                System.out.println(listev.get("start_Date").toString());
    ...
    public ArrayList<Event> getList22() {
        ...
        ConnectionRequest con = new ConnectionRequest();
        con.setUrl(Session.url_server + "cart/AllEvent");
        con.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
            public void actionPerformed(NetworkEvent evt) {
                CartService ser = new CartService();
